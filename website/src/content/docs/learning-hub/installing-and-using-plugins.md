...
| **Canvas Extensions** | Custom canvas views shipped with a plugin (v1.0.79+) | `com.github.copilot/extensions/` |
...
*(v1.0.79+)* **Auto-updating marketplace plugins**: Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to automatically update all installed plugins from that marketplace at session start — no manual `plugin update` required:
...
This is useful for internal plugin registries where you want teams to always run the latest approved versions without any user action.
