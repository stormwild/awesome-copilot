...
| **Extensions** | IDE extensions and canvas extensions installable via the plugin marketplace (v1.0.62+) | `extensions/` or `com.github.copilot/extensions/` |
...
> **Auto-updates for first-party plugins**: First-party plugins (those from the official `copilot-plugins` marketplace) automatically update to the latest version at session start. You don't need to run `copilot plugin update` manually for these.
