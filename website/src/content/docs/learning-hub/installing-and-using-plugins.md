...
*(v1.0.79+)* Add `"autoUpdate": true` to an `extraKnownMarketplaces` entry to **automatically update its plugins at the start of each session** — no manual `copilot plugin update` needed:
...
This is useful for internal marketplaces where you want all team members to always run the latest versions without a manual update step.
