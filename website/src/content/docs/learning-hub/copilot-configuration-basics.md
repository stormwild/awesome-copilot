...
The `/worktree` command (v1.0.61+) creates a new git worktree and switches into it. Use it when you want to spin up a parallel branch and **leave your uncommitted changes behind** in the current worktree:
...
*(v1.0.71+)* If you want to **carry your uncommitted changes into the new worktree**, use `/move` instead:

```
/move my-feature-branch
```

`/move` transfers your in-progress work to the new branch, leaving the original worktree clean. Prior to v1.0.71, `/move` was just an alias for `/worktree`; they now have distinct behavior.
