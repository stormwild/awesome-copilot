...
| `agentStop` | Main agent finishes responding to a prompt | Run final linters/formatters, validate complete changes. *(v1.0.72+)*: If a hook repeatedly blocks (8 consecutive blocks), the CLI forces continuation — hooks receive a `stop_hook_active` flag to detect this and self-limit |
...
> **Preventing infinite block loops (v1.0.72+)**: An `agentStop` hook that always blocks will no longer loop indefinitely. The CLI forces continuation after **8 consecutive blocks**. When this happens, the hook receives a `stop_hook_active: true` flag in its JSON input, so your hook can detect a forced continuation and self-limit (for example, by skipping the blocking check when `stop_hook_active` is `true`):
>
> ```bash
> INPUT=$(cat)
> ACTIVE=$(echo "$INPUT" | jq -r '.stop_hook_active // false')
> if [ "$ACTIVE" = "true" ]; then
>   exit 0  # don't block during forced continuation
> fi
> # ... normal check logic
> ```
