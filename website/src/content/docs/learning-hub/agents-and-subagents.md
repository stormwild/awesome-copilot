...
Yes (v1.0.72+). Multi-turn subagents are now **always enabled**, meaning you can send follow-up messages to a running agent without waiting for it to finish its current response. This lets you provide mid-task guidance or corrections without interrupting the session entirely.
