...
The default maximum nesting depth is **4** (lowered from 6 in v1.0.71) to curb runaway recursive subagent delegation. Usage-based billing users can still adjust `subagents.maxDepth` (up to 128) in `/settings`.

**Can I send follow-up messages to a running subagent?**

Yes. As of **v1.0.72**, multi-turn subagents are always enabled — you can send follow-up messages to a running agent or subagent without any special configuration. This makes interactive steering of long-running tasks much more natural.
