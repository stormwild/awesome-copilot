...
*(v1.0.71+)* The built-in GitHub MCP toolset and tool selection can now be **persisted in `settings.json`**, so your GitHub MCP configuration is preserved across sessions and restarts without requiring you to re-configure it each time.

Use these settings to control which GitHub MCP toolsets and individual tools are enabled:

```json
{
  "githubMcpToolsets": ["repos", "issues", "pull_requests"],
  "githubMcpTools": ["list_issues", "create_pull_request", "search_code"]
}
```

You can also enable all available GitHub MCP tools:

```bash
copilot --add-github-mcp-tool "*"
```

This is equivalent to enabling the full GitHub MCP toolset. These settings are respected in both interactive sessions and prompt mode (`-p`).

> **Note**: The `githubMcpToolsets` and `githubMcpTools` settings only apply to the built-in GitHub MCP server. External GitHub MCP server configurations added via `.mcp.json` are managed separately.
